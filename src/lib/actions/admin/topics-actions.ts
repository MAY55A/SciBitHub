...
export const deleteTopics = async (ids: string[], deletionType: "soft" | "hard" = "soft") => {
    const { error, data: topics } = deletionType === "soft"
        ? await supabase.from("forum_topics").update({ deleted_at: new Date().toISOString() }).in("id", ids).select('id, creator, project, title')
        : await supabase.from("forum_topics").delete().in("id", ids).select('id, creator, project, title');
    if (error) {
        console.log("Database error:", error.message);
